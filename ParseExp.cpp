...
class Calculator {
    ...
    std::stack<Number*> operands_;
    Number* pop () {
        Number* d = operands_.top ();
        operands_.pop ();
        return d;
    }
    void push (Number* d) { operands_.push (d); }
    /* Returns the most recent operation result (top of the operand stack) */
    Number* result () const { return operands_.top (); }
...
protected:
    ...
    void consume(Number* value) { push (value); }
    /* Process an operator token from the input stream */
    void consume(char op) {
        switch (op) {
            case '+':
                //push (pop () + pop ());
                push( calculate->add( pop(), pop()));
                break;
            case '*':
                //push (pop () * pop ());
                push( calculate->multiply( pop(), pop()));
                break;
            case '-':
                {
                    Number* right = pop ();
                    //push (pop () - right);
                    push(calculate->subtract( pop(), right));
                }
                break;
            case '/':
                {
                    Number* right = pop ();
                    //push (pop () / right);
                    push( calculate->divide( pop(), right));
                }
                break;
            /*case '^':
                {
                	Number* left = pop ();
                	//push (left ^ pop ());
                	push( calculate->exponentiate( left, pop()));
                }
                break;*/
            default:
...
public:
     ...
     */
    Number* operate (const std::string& expr) {
...
/*int main () {
    Calculator c;
    std::cout << c.operate ("(20+10)*3/2-3") << std::endl;
    return 0;
}*/
